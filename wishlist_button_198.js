...
function cartCheck(data){
    var cart_btn = document.querySelectorAll('div.addtocart_btn');
    for (var i = 0; i < cart_btn.length; i++) {
        cart_btn[i].addEventListener('load', function() {
            var incart = cart_items.indexOf(this.getAttribute('id'));
            if(((incart)>-1)){
                this.innerHTML = data.CartLabelAfter
                console.log(cart_items.indexOf(this.getAttribute('id')))
                console.log('here ' + this.getAttribute('id'))
            }else{
                this.innerHTML = data.CartLabelBefore
