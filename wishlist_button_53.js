...
function renderButton(data,in_list) {
...
//eventListeners();
    ...
    document.querySelector("select.product-form__variants").addEventListener("change", (event) => {
        var product_id = document.querySelector('select.product-form__variants option[selected="selected"]').getAttribute('value');
...
const serverUrl = "https://192f79c843e8.ngrok.io";

if(window.meta.page.customerId)
console.log(window.meta.page.customerId)
else console.log("customer not loggedIn")
