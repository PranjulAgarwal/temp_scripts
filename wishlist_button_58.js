...
if(window.meta.page.customerId)
{
    ...
    fetch(`${serverUrl}/list_db/${window.location.hostname}/${customer_id}`).then((res)=>{
        return res.json();
    }).then((Customer_data)=>{
...
function renderButton(data,wish_list) {

    var product_id = document.querySelector('select.product-form__variants option[selected="selected"]').getAttribute('value');
    console.log(product_id + "  On Prodct Page" + " in start")

    console.log(wish_list + " inside render")
...
const serverUrl = "https://e2ed944462dc.ngrok.io";
