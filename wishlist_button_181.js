                ...
                fetch(`https://${shop_name}/admin/api/2021-04/products/${productid}.json`,{
                    ...
                    res.json().then((productData)=>{
                        ...
                        const OutofStockCard = `<div class="product_card" id="${productTitle} - ${variantTitle}">
                                                ...
                                                <div id="${variantid}" style="color:grey" data-item="${productTitle} - ${variantTitle}" class="outofstock_btn">
                                                    OUT OF STOCK
                                                </div>
                                            </div>`;
                        if(inventory_quantity>0)                  
                        {
                            if(cart_items.indexOf(variant_id)){
                                document.getElementById('wishlist_grid').innerHTML+= CartItemCard;
                            }
                            else
