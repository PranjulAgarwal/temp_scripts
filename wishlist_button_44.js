...
function changeButton(data,in_list){
    ...
    if(in_list){
        if(data.ButtonType=='1'){
            ...
            var doc = document.createElement(`${data.LabelAftereButton}`)
            btn.childNodes[0].replaceChild(doc,btn.childNodes[0] );
        }
        if(data.ButtonType=='2'){
            btn.style.backgroundColor = `${data.ColorAfterButton}`;
            var doc = document.createElement(`<i class="fa fa-heart" aria-hidden="true"></i>     ${data.LabelAftereButton}`)
            btn.childNodes[0].replaceChild(doc,btn.childNodes[0]);
        }
        ...
        if(data.ButtonType=='5'){
            btn.style.color = `${data.ColorAfterButton}`;
            var doc = document.createElement(`<i class="fa fa-heart" aria-hidden="true"></i>`)
            btn.childNodes[0].replaceChild(doc,btn.childNodes[0]);
        }
    }else{
        if(data.ButtonType=='1'){
            btn.style.backgroundColor = `${data.ColorBeforeButton}`;
            var doc = document.createElement(`${data.LabelBeforeeButton}`)
            btn.childNodes[0].replaceChild(doc,btn.childNodes[0] );
        }
        if(data.ButtonType=='2'){
            btn.style.backgroundColor = `${data.ColorBeforeButton}`;
            var doc = document.createElement(`<i class="fa fa-heart-o" aria-hidden="true"></i>     ${data.LabelBeforeeButton}`)
            btn.childNodes[0].replaceChild(doc,btn.childNodes[0]);
        }
        if(data.ButtonType=='3'){
        ...
        if(data.ButtonType=='5'){
            btn.style.color = `${data.ColorBeforeButton}`;
            var doc = document.createElement(`<i class="fa fa-heart-o" aria-hidden="true"></i>`)
            btn.childNodes[0].replaceChild(doc,btn.childNodes[0]);
        }
    }
